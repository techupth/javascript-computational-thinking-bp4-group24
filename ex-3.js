let orders = 
  ...
  function sortCreditCardType(order,creditCardType){
    let result = [];
    const lowerCaseCreditCardType =creditCardType.toLowerCase(); //convert credit card type to lowercase
    for (let i=0;i<order.length;i++){
        if(order[i].creditCardType.toLowerCase().includes(lowerCaseCreditCardType)){
            result.push(order[i]);
        }
    }
    if (result.length ===0){
  ...
  console.log(sortCreditCardType(orders,"jcb"))
